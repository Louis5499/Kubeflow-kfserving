...
def EndpointSpec(framework, storage_uri, service_account_name = "k8s-sa", transformer_custom_image = ""):
    if framework == 'tensorflow':
        return V1alpha2EndpointSpec(
            ...
            transformer=V1alpha2TransformerSpec(
                min_replicas=1,
                custom=V1alpha2CustomSpec(container=client.V1Container(image=transformer_custom_image, name="kfserving-container"))
            )
        )
...
def deploy_model(action, model_name, default_model_uri, canary_model_uri, canary_model_traffic, namespace, framework, default_custom_model_spec, canary_custom_model_spec, transformer_custom_image, autoscaling_target=0):
    ...
    if framework != 'custom' and default_model_uri:
        default_model_spec = EndpointSpec(framework, default_model_uri, "k8s-sa", transformer_custom_image)
    ...
    # Create Transformer Spec if transformer is included
...
if __name__ == "__main__":
    ...
    parser.add_argument('--transformer-custom-image', type=str, help='Transformer Custom Image', default='')
    ...
    transformer_custom_image = args.transformer_custom_image
    ...
    if kfserving_endpoint:
        ...
        model_status = deploy_model(
            ...
            canary_custom_model_spec=canary_custom_model_spec,
            autoscaling_target=autoscaling_target,
            transformer_custom_image=transformer_custom_image
        )
    print(model_status)
    try:
